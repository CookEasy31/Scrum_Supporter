...
def get_ai_suggestions(problem_description, toc_string, pdf_text):
    ...
    if not API_KEY:
        ...
        return None # Gibt None zurück, um Fehlerzustand anzuzeigen

    genai.configure(api_key=API_KEY)
    model = genai.GenerativeModel(MODEL_NAME)

    # --- System Prompt (Genau wie vom User vorgegeben) ---
    prompt = f"""
...
INHALTSVERZEICHNIS ZUR ORIENTIERUNG:
{toc_string if toc_string else "Inhaltsverzeichnis nicht verfügbar oder nicht detailliert extrahierbar."}

VOLLSTÄNDIGER PDF-INHALT (Ausschnitt):
{pdf_text[:90000]} # Limit leicht erhöht, falls nötig
...
2. Identifiziere anhand des vollständigen PDF-Textes die relevantesten Übungen, die diese Probleme direkt ansprechen:
...
3. Für jeden Übungsvorschlag extrahiere aus dem PDF-Text:
   - Den exakten Titel der Übung.
   - Die exakten Seitenzahlen des gesamten Übungsabschnitts (von Anfang bis Ende).
   - Die exakte Seitenzahl, auf der der "Vorgehen"-Abschnitt beginnt - dieser ist durch die Überschrift "Vorgehen" und eine nummerierte Auflistung von Schritten (01, 02, 03, usw.) erkennbar.
   - Alle im PDF angegebenen Metadaten zur Übung wie Zeitrahmen, Niveau, benötigte Materialien und Rollen.

AUSGABEFORMAT:
Verwende ein stark visuell strukturiertes Format mit Markdown-Elementen:

## 💡 Empfohlene Übung: [EXAKTER ÜBUNGSTITEL AUS DER PDF]

### 📚 Übungsdetails
- **Seiten:** [VON-BIS] (gesamter Übungsabschnitt)
- **Vorgehen-Seite:** [EXAKTE SEITENZAHL] - Hier beginnt die konkrete Schritt-für-Schritt-Anleitung
- **Zeitrahmen:** [EXAKTER ZEITRAHMEN WIE IN DER PDF ANGEGEBEN]
- **Niveau:** [EXAKTES NIVEAU WIE IN DER PDF ANGEGEBEN]
- **Materialien:** [EXAKTE MATERIALIEN WIE IN DER PDF ANGEGEBEN]
- **Rollen:** [EXAKTE ROLLEN WIE IN DER PDF ANGEGEBEN]

---

### 🤔 Warum diese Übung passt:
[KURZE BEGRÜNDUNG IN 1-2 SÄTZEN, WARUM DIESE ÜBUNG DAS BESCHRIEBENE PROBLEM LÖST]

Bei mehreren Übungen wiederhole dieses Format mit einer horizontalen Linie (`---`) als Trenner.

WICHTIG:
- Verwende nur Informationen, die du direkt im PDF-Text gefunden hast.
- Mache KEINE SCHÄTZUNGEN! Gib nur exakte Informationen aus der PDF an.
- Wenn eine Information (z.B. Materialien, Niveau) nicht explizit in der PDF für die Übung angegeben ist, schreibe "Nicht explizit angegeben".
- Die "Vorgehen-Seite" mit der Schritt-für-Schritt-Anleitung ist die wichtigste Information.
...
"""
    ...
    try:
        ...
        # Verbesserte Fehlerbehandlung für API-Antworten
        if not response.parts:
            reason = "Unbekannter Grund"
            if response.prompt_feedback and response.prompt_feedback.block_reason:
                reason = f"Inhaltsblockierung ({response.prompt_feedback.block_reason.name})"
            return f"⚠️ **API-Antwortfehler:** Leere Antwort erhalten. Grund: {reason}. Bitte Problembeschreibung prüfen oder anders formulieren."
...
def initialize_pdf_data(pdf_path):
    """Lädt und verarbeitet die PDF, speichert Daten im Session State."""
    if 'pdf_data_initialized' not in st.session_state:
        with st.spinner(f"Lade und analysiere '{os.path.basename(pdf_path)}'..."):
            full_text, toc_string, toc_error, pdf_error = extract_pdf_data(pdf_path)
            st.session_state.pdf_full_text = full_text
            st.session_state.pdf_toc_string = toc_string
            st.session_state.pdf_toc_error = toc_error
            st.session_state.pdf_processing_error = pdf_error
            st.session_state.pdf_data_initialized = True
            if pdf_error:
                st.error(f"Kritisches Problem beim Laden der PDF: {pdf_error}")
            elif toc_error:
                st.warning(toc_error) # Nur als Warnung anzeigen
        st.rerun() # Neu laden, um Spinner auszublenden und Daten verfügbar zu machen
    # Gib zurück, ob die Initialisierung erfolgreich war (Volltext vorhanden)
    return st.session_state.pdf_full_text is not None
        ...
        if not problem_description or len(problem_description) < 20:
            ...
            with st.spinner("KI analysiert das Problem und sucht nach Übungen..."):
                suggestions = get_ai_suggestions(
                    problem_description,
                    st.session_state.get('pdf_toc_string'),
                    st.session_state.get('pdf_full_text')
                )
